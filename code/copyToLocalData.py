...
def write_train_test_txt(POSITIVE_IMAGES,
    ...
    for img in tqdm(POSITIVE_IMAGES,
        ...
        im = '_'.join(img.split('/')[-2:])
        all_img_fn.append(im)
    ...
    for img in tqdm(NEGATIVE_IMAGES,
        ...
        im = 'neg_' + '_'.join(img.split('/')[-2:])
        copy2(img, os.path.join(OUT_IMAGES_DIR, im))
        all_img_fn.append(im)

    NTRAIN = int(train_ratio * len(all_img_fn))
    TRAIN_IMAGES = all_img_fn[:NTRAIN]
    TEST_IMAGES = all_img_fn[NTRAIN:]
